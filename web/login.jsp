...
<div class="bg-white shadow-lg rounded-lg p-8 max-w-md w-full">
    ...
    <p id="errorMessage" class="text-red-500 text-sm text-center mt-2 hidden">Invalid email or password!</p>
    ...
    <form action="LoginServlet" method="POST" class="mt-6">
        ...
        <!-- Login Button -->
        <button type="submit" class="mt-6 w-full px-4 py-2 bg-yellow-500 text-white font-semibold rounded-lg hover:bg-yellow-600 transition">
            Login
        </button>

        <!-- Register Link -->
        <p class="text-center text-sm text-gray-600 mt-4">
            Don't have an account? <a href="register.jsp" class="text-yellow-500 font-medium hover:underline">Sign up</a>
        </p>
    </form>
</div>
...
<script>
    function togglePassword() {
        ...
        if (input.type === "password") {
            input.type = "text";
            icon.innerHTML = `<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4l16 16M9.9 9.9a5 5 0 017.2 7.2m-2.5-2.5a5 5 0 00-7.2-7.2" />`; // Eye Off Icon
        } else {
            input.type = "password";
            icon.innerHTML = `<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2 12s4-8 10-8 10 8 10 8-4 8-10 8S2 12 2 12z" />`; // Eye Open Icon
        }
    }
</script>
